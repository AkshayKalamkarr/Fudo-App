...
// interface props {
//   fetchMyRestaurent: () => Promise<void>;
// }

// const AddRestaurent = ({ fetchMyRestaurent }: props) => {
...
//     try {
//       setSubmitting(true);
//       await axios.post(`${restaurentService}/api/restaurant/new`, formData, {
//         headers: {
...
//       <div className="mx-auto max-w-lg rounded-xl bg-white p-6 shoadow-sm space-y-5">
...
//         >
//           {submitting ? "submitting.." : "add restaurent"}
//         </button>
//       </div>
//     </div>
//   );
...
const AddRestaurent = ({ fetchMyRestaurent }: props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Restaurent added successfully");
      await fetchMyRestaurent(); // ✅ Refresh restaurant list after success
    } catch (error: any) {
      toast.error(error?.response?.data?.message || "Something went wrong");
    } finally {
      setSubmitting(false);
  ...
  // ✅ Removed stray toast.success() that was firing on every render
